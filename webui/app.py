...
def main():
    ...
    production_mode = st.sidebar.toggle(
        "🚀 本番環境モード", 
        value=st.session_state.get('production_mode', False),
    ...
    if not production_mode:
        # Webプロトタイプモード: Web機能のみ表示
        menu_items.extend(web_prototype_items)
    else:
        # 本番環境モード: 全機能表示
        menu_items.extend(production_items)
        menu_items.extend(web_prototype_items)
    ...
    if menu == "キャラクター状態":
        ...
        if production_mode:
            ...
            show_feature_locked("配信設定")
    elif menu == "Live2D・OBS連携":
        if production_mode:
            show_streaming_integration()
        else:
            show_feature_locked("Live2D・OBS連携")
    elif menu == "🌈 Webプロトタイプ":
        show_web_prototype()
    elif menu == "📊 感情ダッシュボード":
...
def show_feature_locked(feature_name):
    ...
    if st.button("🚀 本番環境モードを有効にする", type="primary"):
